...
class CourseController extends Controller
{
    ...
    public function loadCoursePayments(Request $request)
    {
        ...
        if ($request->input('payment_id_update')) {
            ...
            // Include Customer Relation Executive (CRE) info so frontend can populate the field
            $return_data['customer_relation_executive'] = $payment_data->customer_relation_executive;
            $creUser = null;
            if ($payment_data->customer_relation_executive) {
                $creUser = \App\Models\User::find($payment_data->customer_relation_executive);
            }
            $return_data['cre_name'] = $creUser ? ($creUser->name) : null;
        }
